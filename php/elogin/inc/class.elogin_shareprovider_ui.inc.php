...
class elogin_shareprovider_ui {
	...
	public function share_provider_edit($content=null) {
		...
		if( isset($content['button']) && isset($content['button']['save']) ) {
			...
			$provider->setCollectiveUserShare($content['collective_usershare']);
			$provider->setCollectiveGroupShare($content['collective_groupshare']);
			$provider->setAutoAddUserGroup($content['auto_adduser_group']);
		...
		if( $provider ) {
			...
			$content['auto_adduser_group']	= $provider->getAutoAddUserGroup();
